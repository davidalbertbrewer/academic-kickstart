...
<br>
<ul>
<li>The sessions were **four hours long**, including a snacktime :) 
<li>He taught how to **code Python from the command line and [IDLE](https://docs.python.org/3/library/idle.html)**....  
*IDLE is that "GUI" Python installs "which has been bundled with the default implementation of the language since 1.5.2b1" according to the Wikipedia page.*
<br>
![](C:\Users\dbrewer\PROJECTS\academic-kickstart\static\img\start-python-idle-in-windows.png)
...
<blockquote>
**Thanks to John and Christian, from then on I was hooked on data.**
</blockquote>
So what better way to start this blog than to revisit an example from Dataclysm and the [OKCupid blog](https://theblog.okcupid.com/the-big-lies-people-tell-in-online-dating-a9e3990d6ae2).  
...
![](C:\Users\dbrewer\PROJECTS\academic-kickstart\static\img\male_height_distribution_on_okcupid.JPG)  
<br>
...
![](C:\Users\dbrewer\PROJECTS\academic-kickstart\static\img\male_height_distribution_on_okcupid_2.JPG)  
<br>
![](C:\Users\dbrewer\PROJECTS\academic-kickstart\static\img\female_height_distribution_on_okcupid.png) 
<br>
<br>
<blockquote>
**I wonder is Cristian's claim always true?  Or is this an anomaly within online dating website data?**
![](C:\Users\dbrewer\PROJECTS\academic-kickstart\static\img\height_exaggeration.jpg) 
</blockquote>
...
<blockquote>
![](C:\Users\dbrewer\PROJECTS\academic-kickstart\static\img\normal_distribution_probability_density_function.JPG)  
...
![](C:\Users\dbrewer\PROJECTS\academic-kickstart\static\img\68-95-99-rule.png)


The PDF can be re-written this way:  
![](C:\Users\dbrewer\PROJECTS\academic-kickstart\static\img\normal_distribution_probability_density_function_2.JPG)  
And then simplified assuming $\mu$ = 0 and $\sigma$ = 1   
![](C:\Users\dbrewer\PROJECTS\academic-kickstart\static\img\normal_distribution_probability_density_function_2_simplified.JPG)  
<br>
...
So this is what is going on under the hood of the dnorm function in R which I will use below to complete this analysis.  
<br>
<blockquote>
...
<br>
<blockquote>
*I did discover an interesting longitudinal dataset of heights across nearly 100 countries collected by the DHS Program and made available on their website [here](https://www.dhsprogram.com/What-We-Do/survey-search.cfm?pgtype=main&SrvyTp=country) but unfortunately the U.S. is not included in these data.*
</blockquote>
...
![](C:\Users\dbrewer\PROJECTS\academic-kickstart\static\img\male-height-dist-brfss.png)
![](C:\Users\dbrewer\PROJECTS\academic-kickstart\static\img\female-height-dist-brfss.png)
...
heights_df$label <- paste(heights_df$feet, "\'", heights_df$inches, "\"", sep="")
```
<br>
...
<br>
<br>
**Creating a similar analysis with the actual reported heights compared to the expected normal distribution given the mean and standard deviation of the sample**
<ul>
...
df <- heights_df_table_by_label_merged[heights_df_table_by_label_merged$sex == sex_for_analysis,]
...
df_test$cond <- "actual"
set.seed(1983)
mean <- as.numeric(heights_df_summary[heights_df_summary$sex == sex_for_analysis, "mean"])
sd <- as.numeric(heights_df_summary[heights_df_summary$sex == sex_for_analysis, "sd"])
xseq<-seq(55,83,1)
...
```
<br>
**ggplot2 to create the viz similar to OKCupid theme**
<ul>
...
ggplot(data = subset(df_test, cond == "dnorm")
       , aes(x=label, y=percent, group = 1, colour = cond)) +
        ...
        scale_x_discrete(name =""
                         ...
                         , labels = as.character(df_test$label[seq(0,length(df$label),2)])
                         , limits = df_test$label[df_test$cond== "dnorm"]) +
        scale_y_continuous(breaks=seq(min(df_test$percent),max(df_test$percent),4)
